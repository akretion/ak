...
class AkSuggest(AkSub):
    ...
    config = cli.SwitchAttr(["c", "config"], default=SPEC_YAML, help="Config file")
    xand = cli.Flag(
        ["x", "xand"],
    ...
    def _set_suggested(self):
        ...
        for key, branch in spec.items():
            ...
            if branch.get("useless"):
                modules = [x for x in modules if x not in branch.get("useless")]
            if self._filter_according_branch(branch, modules):
                if not branch.get("src") and len(spec[key].get("remotes")) > 0:
                    branch_name = [y for x, y in spec[key]["remotes"].items()][0]
                elif branch.get("src"):
